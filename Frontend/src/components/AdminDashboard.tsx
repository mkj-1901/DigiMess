...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [statsRes, optOutsRes, rebatesRes, reviewsRes, attendanceRes, summaryRes] = await Promise.all([
  ...
  const handleApproveReview = async (id: string, approved: boolean) => {
    try {
      ...
      // Refresh
      const reviewsRes = await adminService.getReviews();
      if (reviewsRes.success && reviewsRes.data) setReviews(reviewsRes.data);
    } catch (err) {
          ...
          <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl p-8 border border-gray-100">
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-5 w-0 flex-1">
                    <dl>
                      ...
                      <dd className="text-3xl font-bold text-gray-900">{new Set(attendance.filter(att => att.date === today).map(att => typeof att.user === 'object' ? att.user.id : att.user)).size}</dd>
                    </dl>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg shadow overflow-hidden">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {/* Pending Opt-Outs */}
                    {optOuts.filter(opt => opt.approved === false).map((opt) => (
                      <tr key={`optout-${opt._id}`} className="bg-blue-50">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-blue-800">Opt-Out</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{typeof opt.user === 'object' ? opt.user.name : opt.user}</td>
                        <td className="px-6 py-4 text-sm text-gray-900">
                          {new Date(opt.startDate).toLocaleDateString()} - {new Date(opt.endDate).toLocaleDateString()}<br/>
                          <span className="text-xs text-gray-500">{opt.reason.substring(0, 50)}{opt.reason.length > 50 ? '...' : ''}</span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{opt.createdAt ? new Date(opt.createdAt).toLocaleDateString() : 'N/A'}</td>
                    ...
                    {/* Pending Rebates */}
                    {rebates.filter(reb => reb.status === 'pending').map((reb) => (
                      <tr key={`rebate-${reb._id}`} className="bg-green-50">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-green-800">Rebate</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{typeof reb.user === 'object' ? reb.user.name : reb.user}</td>
                        <td className="px-6 py-4 text-sm text-gray-900">
                          Month: {reb.monthYear}<br/>
                          <span className="text-xs text-gray-500">Amount: ₹{reb.calculatedAmount}</span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{reb.monthYear || 'N/A'}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <button
                    ...
                    {/* Pending Reviews */}
                    {reviews.filter(rev => rev.approved === false).map((rev) => (
                      <tr key={`review-${rev._id}`} className="bg-purple-50">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-purple-800">Review</td>
                        ...
                        <td className="px-6 py-4 text-sm text-gray-900">
                          ...
                          <span className="text-xs text-gray-500">{rev.comment ? rev.comment.substring(0, 50) + (rev.comment.length > 50 ? '...' : '') : 'No comment'}</span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{rev.createdAt ? new Date(rev.createdAt).toLocaleDateString() : 'N/A'}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <button
                    ...
                    {optOuts.filter(opt => opt.approved === false).length === 0 && 
                     rebates.filter(reb => reb.status === 'pending').length === 0 && 
                     reviews.filter(rev => rev.approved === false).length === 0 && (
                      <tr>
                        <td colSpan={5} className="px-6 py-8 text-center text-gray-500">
                          <div className="flex flex-col items-center">
                            ...
                            <p className="text-lg font-medium">No pending requests to approve</p>
            ...
            <section className="mt-12">
              ...
              >
                ...
                <svg
                  className={`h-5 w-5 text-gray-500 transform transition-transform duration-200 ${showAttendance ? 'rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <div className="mt-4 bg-white rounded-lg shadow overflow-hidden">
                  <div className="p-4 bg-blue-50 border-l-4 border-blue-400">
                    <p className="text-sm text-blue-700">
                      View recent meal attendance records for all students. This helps track daily participation and monitor mess usage patterns.
                    </p>
                  </div>
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
            ...
            <section className="mt-12">
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <h3 className="text-xl font-bold text-gray-900">Opt-Out Meals</h3>
                </div>
                <svg
                  className={`h-5 w-5 text-gray-500 transform transition-transform duration-200 ${showOptOuts ? 'rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              {showOptOuts && (
                <div className="mt-4 bg-white rounded-lg shadow overflow-hidden">
                  <div className="p-4 bg-blue-50 border-l-4 border-blue-400">
                    <p className="text-sm text-blue-700">
                      View all opt-out requests from students. This includes both approved and pending requests to help track meal preferences and absences.
                    </p>
                  </div>
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {optOuts.map((opt) => (
                        <tr key={opt._id}>
                          ...
                          <td className="px-6 py-4 text-sm text-gray-900">{opt.reason}</td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                              opt.approved ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                            }`}>
            ...
            <section className="mt-12">
              ...
              >
                ...
                <svg
                  className={`h-5 w-5 text-gray-500 transform transition-transform duration-200 ${showReviews ? 'rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <div className="mt-4 bg-white rounded-lg shadow overflow-hidden">
                  <div className="p-4 bg-purple-50 border-l-4 border-purple-400">
                    <p className="text-sm text-purple-700">
                      View all student reviews and feedback. This includes both approved and pending reviews to monitor food quality and student satisfaction.
                    </p>
                  </div>
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Student</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Meal Date</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {reviews.map((rev) => (
                        <tr key={rev._id}>
                          ...
                          <td className="px-6 py-4 text-sm text-gray-900">{rev.comment || 'No comment'}</td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
            ...
            <section className="mt-12">
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <h3 className="text-xl font-bold text-gray-900">Mess Rebates</h3>
                </div>
                <svg
                  className={`h-5 w-5 text-gray-500 transform transition-transform duration-200 ${showRebates ? 'rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <div className="mt-4 bg-white rounded-lg shadow overflow-hidden">
                  <div className="p-4 bg-green-50 border-l-4 border-green-400">
                    <p className="text-sm text-green-700">
                      View all rebate requests and their processing status. This helps track financial adjustments and student billing corrections.
                    </p>
                  </div>
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {rebates.map((reb) => (
                        <tr key={reb._id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-green-600">₹{reb.calculatedAmount}</td>
                          <td className="px-6 py-4 text-sm text-gray-900">Rebate for {reb.monthYear}</td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                              reb.status === 'approved' ? 'bg-green-100 text-green-800' :
                              reb.status === 'rejected' ? 'bg-red-100 text-red-800' :
                              'bg-yellow-100 text-yellow-800'
                            }`}>
                              {reb.status === 'approved' ? 'Approved' :
                               reb.status === 'rejected' ? 'Rejected' : 'Pending'}
                            </span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{reb.createdAt ? new Date(reb.createdAt).toLocaleDateString() : 'N/A'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="bg-white rounded-lg shadow p-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="text-center">
                    <div className="text-3xl font-bold text-blue-600">{reviewSummary?.averageRating?.toFixed(1) || 'N/A'}</div>
                    <div className="text-sm text-gray-500">Average Rating</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl font-bold text-green-600">{reviewSummary?.totalReviews || 0}</div>
                    <div className="text-sm text-gray-500">Total Reviews</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl font-bold text-purple-600">{reviews.filter(r => r.approved).length}</div>
                    <div className="text-sm text-gray-500">Approved Reviews</div>
                  </div>
                </div>
                {reviewSummary?.topComments && reviewSummary.topComments.length > 0 && (
                  <div className="mt-6">
                    <h4 className="text-lg font-semibold mb-4">Top Approved Comments</h4>
                    <div className="space-y-4">
                      {reviewSummary.topComments.slice(0, 3).map((comment: string, index: number) => (
