...
import type { User, MealAttendance, Rebate, Review, OptOut } from '../types/User';
          ...
          <div className="flex justify-between h-16">
            <div className="flex items-center">
              <div className="flex items-center space-x-3">
                ...
                <h1 className="text-xl font-bold text-gray-900">DigiMess Student Portal</h1>
              </div>
            </div>
          ...
          <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl p-8 border border-white/20">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 text-lg">
                Manage your meals, track rebates, and share your feedback effortlessly.
              </p>
            </div>
            ...
            <section className="mb-12">
              <div className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white p-6 rounded-xl shadow-lg mb-6">
                ...
                <p className="text-blue-100">Log your daily meals and track your attendance</p>
              </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
                      <tr>
                        ...
                        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Meals</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                      {attendanceHistory.slice(-5).reverse().map((att) => (
                        <tr key={att._id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{new Date(att.date).toLocaleDateString()}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <div className="flex space-x-2">
                              {att.breakfast && <span className="px-2 py-1 bg-orange-100 text-orange-800 rounded-full text-xs font-medium">B</span>}
                              {att.lunch && <span className="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium">L</span>}
                              {att.dinner && <span className="px-2 py-1 bg-purple-100 text-purple-800 rounded-full text-xs font-medium">D</span>}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{att.totalMeals}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <section className="mb-12">
              <div className="bg-gradient-to-r from-green-500 to-emerald-600 text-white p-6 rounded-xl shadow-lg mb-6">
                ...
                <p className="text-green-100">Calculate and track your monthly meal rebates</p>
              </div>
              <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                <button
                ...
                {currentRebate && (
                  <div className="mt-6 p-6 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200">
                    <h3 className="font-bold text-lg text-gray-900 mb-4">Rebate for {currentRebate.monthYear}</h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="text-center">
                        <p className="text-2xl font-bold text-green-600">₹{currentRebate.calculatedAmount}</p>
                        <p className="text-sm text-gray-600">Amount</p>
                      </div>
                      <div className="text-center">
                        <p className="text-2xl font-bold text-blue-600">{currentRebate.attendedDays}</p>
                        <p className="text-sm text-gray-600">Attended</p>
                      </div>
                      <div className="text-center">
                        <p className="text-2xl font-bold text-orange-600">{currentRebate.optOutDays}</p>
                        <p className="text-sm text-gray-600">Opt-out</p>
                      </div>
                      <div className="text-center">
                        <p className="text-2xl font-bold text-red-600">{currentRebate.missedDays}</p>
                        <p className="text-sm text-gray-600">Missed</p>
                      </div>
                    </div>
                    <div className="mt-4 text-center">
                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${currentRebate.status === 'approved' ? 'bg-green-100 text-green-800' : currentRebate.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                      {rebates.slice(-5).reverse().map((reb) => (
                        <tr key={reb._id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{reb.monthYear}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-green-600">₹{reb.calculatedAmount}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <span className={`px-3 py-1 rounded-full text-xs font-medium ${reb.status === 'approved' ? 'bg-green-100 text-green-800' : reb.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>
                              {reb.status}
                            </span>
                          </td>
                        </tr>
            ...
            <section className="mb-12">
              <div className="bg-gradient-to-r from-purple-500 to-pink-600 text-white p-6 rounded-xl shadow-lg mb-6">
                ...
                <p className="text-purple-100">Share your feedback and help improve our meals</p>
              </div>
              <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                      {reviews.slice(-5).reverse().map((rev) => (
                        <tr key={rev._id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{new Date(rev.mealDate).toLocaleDateString()}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <div className="flex items-center">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <span className={`px-3 py-1 rounded-full text-xs font-medium ${rev.approved ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
            ...
            <section className="mb-12">
              <div className="bg-gradient-to-r from-red-500 to-pink-600 text-white p-6 rounded-xl shadow-lg mb-6">
                ...
                <p className="text-red-100">Request to skip meals for specific dates</p>
              </div>
              <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                  <div>
              ...
              <div className="mt-8">
                ...
                <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                      {optOuts.slice(-5).reverse().map((opt) => (
                        <tr key={opt._id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{new Date(opt.startDate).toLocaleDateString()}</td>
